package cn.iutils.common.utils.excel;

import cn.iutils.common.utils.excel.handler.ExcelHeader;
import cn.iutils.common.utils.excel.handler.ExcelTemplate;
import cn.iutils.common.utils.excel.utils.BeanUtilsEx;
import cn.iutils.common.utils.excel.utils.Utils;
...
public class ExcelUtils {
    ...
    private <T> List<T> readExcel2ObjectsHandler(Workbook workbook, Class<T> clazz, int offsetLine, int limitLine,
        ...
        for (int i = offsetLine + 1; i <= maxLine; i++) {
            ...
            for (Cell cell : row) {
                ...
                BeanUtilsEx.copyProperty(obj, filed, value);
            }
            list.add(obj);
        }
        return list;
    ...
    private ExcelTemplate exportExcelByModuleHandler(String templatePath, int sheetIndex, List<?> data,
        ...
        for (Object object : data) {
            ...
            for (ExcelHeader header : headers) {
                templates.createCell(BeanUtilsEx.getProperty(object, header.getFiled()), null);
            }
        }
        return templates;
    }
    ...
    private ExcelTemplate exportExcelByModuleHandler(String templatePath, int sheetIndex, Map<String, List> data,
        ...
        for (Map.Entry<String, List> entry : data.entrySet()) {
            for (Object object : entry.getValue()) {
                ...
                for (ExcelHeader header : headers) {
                    templates.createCell(BeanUtilsEx.getProperty(object, header.getFiled()), entry.getKey());
                }
            }
        }
    ...
    private Workbook exportExcelNoModuleHandler(List<?> data, Class clazz, boolean isWriteHeader, String sheetName,
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            for (int j = 0; j < headers.size(); j++) {
                row.createCell(j).setCellValue(BeanUtilsEx.getProperty(_data, headers.get(j).getFiled()));
            }
        }
        return workbook;
    }
