                                ...
                                <div class="am-btn-toolbar">
                                    <div class="am-btn-group am-btn-group-xs">
                                        <button type="button" class="am-btn am-btn-default am-btn-success"
                                                onclick="openModel(false,'${ctx}/mt/userInfo/create')"><span
                                ...
                                <table id="contentTable"
                                    ...
                                    <c:forEach items="${page.list}" var="userInfo" varStatus="status">
                                        <tr>
                                            ...
                                            <td>${userInfo.wx}</td>
                                            <c:choose>
                                                <c:when test="${userInfo.identity==0}">
                                                    <td>学生</td>
                                                </c:when>
                                                <c:when test="${userInfo.identity==1}">
                                                    <td>老师</td>
                                                </c:when>
                                                <c:when test="${userInfo.identity==2}">
                                                    <td>教官</td>
                                                </c:when>
                                                <c:otherwise>
                                                    <td>未知</td>
                                            ...
                                            <td>
                                                <a href="javascript:;"
                                                   onclick="openModel('修改用户表','${ctx}/mt/userInfo/update?id=${userInfo.id}')"
                                                   title="编辑"><span class="am-icon-pencil"></span></a>
                                                <a href="${ctx}/mt/userInfo/${userInfo.id}/delete?pageNo=${page.pageNo}&pageSize=${page.pageSize}"
